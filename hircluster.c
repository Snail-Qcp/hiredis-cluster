    ...
    const char *ip, int port)
{
    ...
    if(cc->connect_timeout){
        c = redisConnectWithTimeout(ip, port, *cc->connect_timeout, cc->szAuth);
    }else{
        c = redisConnect(ip, port, cc->szAuth);
    }
...
void redisClusterPing(redisClusterContext *cc) {
    ...
    while((de = dictNext(di)) != NULL)
    {
        ...
        if (NULL == reply || cc->err != REDIS_OK) {
            if (REDIS_OK == redisReconnect(c)) {
...
redisClusterContext *redisClusterConnect(const char *addrs, int flags, const char *auth)
{
    redisClusterContext *cc;
    ...
    if (NULL != auth) {
        strncpy(cc->szAuth, auth, sizeof(cc->szAuth) - sizeof(char));
    }
...
redisClusterContext *redisClusterConnectWithTimeout(
    const char *addrs, const struct timeval tv, int flags, const char *auth)
{
    redisClusterContext *cc;
...
redisClusterContext *redisClusterConnectNonBlock(const char *addrs, int flags, const char *auth) {
...
redisContext *ctx_get_by_node(redisClusterContext *cc, cluster_node *node)
{
    ...
    if(cc->connect_timeout)
    {
        c = redisConnectWithTimeout(node->host, node->port, *cc->connect_timeout, cc->szAuth);
    }
    else
    {
        c = redisConnect(node->host, node->port, cc->szAuth);
    }
...
redisClusterAsyncContext *redisClusterAsyncConnect(const char *addrs, int flags) {
    ...
    cc = redisClusterConnectNonBlock(addrs, flags, NULL);
    if(cc == NULL)
    {
        return NULL;
    }
